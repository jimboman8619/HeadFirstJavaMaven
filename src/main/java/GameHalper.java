...
public class GameHalper {
    ...
    public ArrayList<String> placeDotCom(int comSize) {
        ...
        while (!success & attempts++ < 200) { // Главный поисковый цикл
            ...
            while (success && x < comSize) { // Ищем соседнюю использованную ячейку
        ...
        } // Конец главного поискового цикла

        int x = 0; // Переводим местоположение в символьные координаты
        int row = 0;
        int column = 0;
        while (x < comSize) {
            grid[coords[x]] = 1; // Помечаем ячейку на главной сетке как использованные
